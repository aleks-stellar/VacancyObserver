...
- модуль file_worker реализует абстрактный класс FileWorker;
  наследники которого производят чтение по указанным критериям, запись в файл и удаление
  из файла информации о вакансиях;
- модуль json_worker реализует класс JSONWorker для работы с файлами типа JSON;
...
3. Определить абстрактный класс, который обязывает реализовать методы для добавления вакансий в файл, получения данных
   ...
   CSV- или Excel-файлом, с TXT-файлом. Данный класс выступит в роли основы для коннектора, заменяя который (
   класс-коннектор), можно использовать в качестве хранилища одну из баз данных или удаленное хранилище со своей
   ...
   пригодятся для интеграции к БД. Сделайте заглушку в коде.
